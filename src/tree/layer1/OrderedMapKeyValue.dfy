include "../../../src/tree/layer1/MapKeyValue.dfy"
...
trait OrderedMapKeyValueIterator extends MapKeyValueIterator{
...
function method HasPrev(): bool//igual que HasNext
  ...
  method Copy() returns (it: MapKeyValueIterator)
    ...
    ensures it is OrderedMapKeyValueIterator
    ensures it.Valid()
...
trait OrderedMapKeyValue extends MapKeyValue {
  
 
 function Iterators(): set<MapKeyValueIterator>
    reads this, Repr()
    requires Valid()
    ensures forall it | it in Iterators() :: it in Repr() && it.Parent() == this
    ensures forall it | it in Iterators() :: it is OrderedMapKeyValueIterator

method First() returns (it: MapKeyValueIterator)
    modifies this, Repr()
    requires Valid()
  ...
  method Last() returns (it: OrderedMapKeyValueIterator)//iterator to the last element
    modifies this, Repr()
    requires Valid()
    requires forall x | x in Repr() :: allocated(x)
    ensures Valid()
...
method find(k:K) returns (newt:MapKeyValueIterator)
    modifies this, Repr()
    requires Valid()
    requires forall x | x in Repr() :: allocated(x)
    ensures Valid() 
    ...
    ensures newt is OrderedMapKeyValueIterator
    ensures fresh(newt) 
  ...
  method insert(mid: MapKeyValueIterator, k: K, v : V) returns (newt:MapKeyValueIterator)
    modifies this, Repr()
    requires Valid()
    requires mid.Valid() 
  ...
  method erase(mid:MapKeyValueIterator) returns (next: MapKeyValueIterator)
    modifies this, Repr()
    requires Valid()
    requires mid.Valid()
    requires mid.Parent() == this
    ...
    ensures next is OrderedMapKeyValueIterator
    ensures fresh(next)
