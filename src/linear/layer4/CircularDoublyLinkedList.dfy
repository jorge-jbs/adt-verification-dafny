...
class CNode<A(0)> {
  ...
  constructor(prev: CNode<A>, data: A, next: CNode<A>)
...
class CircularDoublyLinkedList<A(0)> {
  var phantom: CNode<A>;
  ...
  static lemma ModelRelationWithSpineAux(spine: seq<CNode<A>>, model: seq<A>)
    ...
    ensures forall n | n in old(spine) :: n in spine
  {
    ...
    { //GHOST
      assert spine[(i + |spine| - 1) % |spine|] == mid.prev;
      NextPrevIn(mid);
    }
    mid.prev.next := n;
    mid.prev := n;
