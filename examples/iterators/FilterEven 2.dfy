...
requires -1<=i<=|xs|
ensures i == -1 || i==|xs|  ==> validIt(xs,i,f)
ensures 0 <= i<|xs| && f(xs[i]) ==> validIt(xs,i,f)
{
  (i==|xs|) || (i==-1) || (i<|xs| && f(xs[i]))
}
...
method {:timeLimitMultiplier 100} filterEven(l:LinkedList<int>) 
  ...
  ensures forall iter | iter in old(l.Iterators()) && old(iter.Valid()) && validIt(old(l.Model()),old(iter.Index()),x => x%2==0) 
         && old(iter.Index())>=0 ::iter.Valid() && iter.Parent()==old(iter.Parent()) && 
         iter.Index()==|FilterR(old(l.Model())[..old(iter.Index())],x => x%2==0)| 
{
  ...
  while b
    ...
    invariant it.Valid() && 0<=it.Index()<=|l.Model()|
  ...
  assert forall iter | iter in old(l.Iterators()) && old(iter.Valid()) && validIt(old(l.Model()),old(iter.Index()),x => x%2==0) && old(iter.Index())>=0
