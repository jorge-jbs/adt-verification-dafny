...
lemma {:verify false} DupDupRev<A>(xs: seq<A>)
...
lemma {:induction xs} {:verify false} setDup<A>(xs: seq<A>)
ensures forall x :: x in xs <==> x in Dup(xs)
ensures |Dup(xs)|==2*|xs|
ensures forall i | 0<=i<|xs| :: xs[i]==Dup(xs)[2*i]==Dup(xs)[2*i+1]
{}

 lemma {:induction xs} {:verify false} dupElsAux<A>(xs: seq<A>,x:A)
 requires x in xs
...
lemma {:induction xs} {:verify false} dupEls<A>(xs: seq<A>)
  ...
  ensures forall x | x in l.Repr() :: allocated(x)
{
  ...
  while it.HasNext()
    ...
    invariant forall iter | iter in old(l.Iterators()) && old(iter.Valid()) && old(iter.Index()) in mit::
       iter.Valid() && mit[old(iter.Index())]==iter.Index()
  ...
  ensures forall x | x in l.Repr() :: allocated(x)
{
  ...
  while it.HasNext()
    ...
    invariant forall iter | iter in old(l.Iterators()) && old(iter.Valid()) && old(iter.Index()) in mit::iter.Valid() && mit[old(iter.Index())]==iter.Index();
